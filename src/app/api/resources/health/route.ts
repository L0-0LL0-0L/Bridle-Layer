...
import { z } from "zod";
import { healthCheckResource } from "@/lib/resource-execution";
...
export async function POST(request: Request) {
  ...
  if (parsed.data.providerUserId !== parsed.data.resource.ownerId) {
    return NextResponse.json({ error: "Provider-only health probe rejected." }, { status: 403 });
  }

  const result = await healthCheckResource({
